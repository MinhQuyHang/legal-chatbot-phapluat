...
==================================================================
PIPELINE NÂNG CẤP:
  Câu hỏi → PhoBERT classifier → chapter → dense search → PhoGPT → trả lời

CÁCH DÙNG TRÊN COLAB:
  1. Upload file này + data/classification.csv lên Colab
  2. Chọn Runtime → Change runtime type → T4 GPU
  3. Chạy: !python train_model.py

SAU KHI TRAIN XONG:
  - Download thư mục model/phobert_classifier/ về máy
  - Dùng predict.py để chạy local trên CMD

SO VỚI PHIÊN BẢN CŨ (TF-IDF + SVM):
  [THAY] TfidfVectorizer     → PhoBERT Tokenizer
  [THAY] LinearSVC           → PhoBERT + Classification Head
  [THAY] pickle .pkl         → save_pretrained() → thư mục
  [GIỮ]  Stratified 70/15/15 split
  [GIỮ]  Macro F1, Cohen's Kappa, Confusion Matrix
  [GIỮ]  RAG threshold search
  [GIỮ]  write_config_yaml()
"""
