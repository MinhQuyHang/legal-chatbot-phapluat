...
class LegalChatChain:
    ...
    def __init__(
        ...
        top_k_docs: int = 3,
        memory_k: int = 5,
        max_chunk_length: int = 400,   # None = không giới hạn
    ...
    def _clean_response(text: str) -> str:
        ...
        # 2. Cắt tại các marker prompt bị echo lại
        cut_markers = ["Câu hỏi:", "Yêu cầu:", "Câu hỏi của sinh viên:", 
                    "Dựa vào tài liệu sau:", "Tài liệu tham khảo:", "Hãy trả lời"]
        for marker in cut_markers:
            if marker in text:
                text = text[:text.find(marker)].strip()

        # 3. Cắt "Trả lời:" ở bất kỳ vị trí nào
        text = re.sub(r'\s*Trả lời\s*:\s*', ' ', text, flags=re.IGNORECASE).strip()

        # 4. Xử lý streaming lỗi bắt đầu bằng "Dang"
