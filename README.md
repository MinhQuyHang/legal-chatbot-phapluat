...
[![Static Badge](https://img.shields.io/badge/Ollama-Qwen2.5%3A7b-green?logo=ollama&logoColor=green&labelColor=white)](https://ollama.com/library/qwen2.5)
...
└─────────────────────────────────────────────┘
        ...
        ▼
┌───────────────────────────────────────────────────────┐
│  Tầng 3 — Qwen2.5:7b (Ollama)                         │
│  • Prompt định dạng ChatML (<|im_start|> / <|im_end|>)│
...
### Tầng 3 — Sinh câu trả lời (Qwen2.5:7b)

Các đoạn văn được truy xuất ghép thành một khối ngữ cảnh và đưa vào prompt theo định dạng **ChatML** (`<|im_start|>system`, `<|im_start|>user`, `<|im_start|>assistant`) — định dạng chuẩn mà Qwen2.5 được fine-tune. System prompt yêu cầu model chỉ trả lời dựa trên ngữ cảnh được cung cấp, và trả lời đúng một câu fallback cố định nếu ngữ cảnh không đủ thông tin.

Mỗi chunk được giới hạn còn 600 ký tự trước khi đưa vào prompt để kiểm soát độ trễ và tránh vượt quá context window hiệu dụng.

Đầu ra được lọc qua bộ hậu xử lý loại bỏ các artifact định dạng ChatML còn sót lại (`<|im_start|>`, `<|im_end|>` và các marker tương tự). Cơ chế phát hiện hallucination so sánh câu trả lời với lịch sử hội thoại, tự động fallback về đoạn RAG liên quan nhất nếu phát hiện model đang lặp lại câu trả lời cũ.
...
│
├── llm/
│   ├── chain.py              # LegalChatChain (ChatML prompt, streaming, fallback)
...
### Bước 3 — Tải mô hình ngôn ngữ Qwen2.5:7b
...
Model sử dụng định dạng ChatML gốc của Qwen2.5, không cần tạo Modelfile tùy chỉnh.
...
> - `data/rag_data.json` → [Google Drive](https://drive.google.com/file/d/1OFytX2ObvYCWc6KaNhigbDA_FkFBiP9V/view?usp=sharing)
> - `data/faiss_index.bin` → [Google Drive](https://drive.google.com/file/d/1jkYPLENhpVnPMjrTzyFjRTlPMpqZ-mlx/view?usp=sharing)
> - `data/faiss_meta.json` → [Google Drive](https://drive.google.com/file/d/1wpXE9rQPXWjrPfysX1pJEJpNGVcdlSw1/view?usp=sharing)
...
| `max_chunk_length` | 600 | Số ký tự tối đa của mỗi chunk đưa vào prompt LLM |
...
**Giới hạn context window.** Với 3 chunk × 600 ký tự ≈ 1.800 ký tự context, các câu hỏi đòi hỏi tổng hợp từ nhiều mục trong giáo trình có thể nhận được câu trả lời chưa đầy đủ.
...
3. Hui, B. et al. (2024). [Qwen2.5: A Party of Foundation Models](https://arxiv.org/abs/2412.15115).
